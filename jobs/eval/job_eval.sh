...
#
# Examples:
#   sbatch jobs/eval/job_eval.sh
#   EMBEDDINGS=embeddings_up.pt sbatch jobs/eval/job_eval.sh
...
python scripts/train/frozen_baseline.py \
    --dataset "${DATASET}" --embed_model "${EMBED_MODEL}" --device cuda

echo "=================== geometry floor ==================="
if [ -n "${EMBEDDINGS:-}" ]; then
    python scripts/analysis/eval_structure.py \
