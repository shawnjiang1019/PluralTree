...
def load_grailqa(
    ...
    Expected files under ``data_dir`` (fetched by ``scripts/fetch/get_grailqa.py``):
    ...
    if not train_rows or not dev_rows:
        raise FileNotFoundError(
            ...
            f"`python scripts/fetch/get_grailqa.py --out {data_dir}` (needs network)."
        )
...
from data.loaders.culturalbench import compute_text_embeddings  # noqa: E402,F401
